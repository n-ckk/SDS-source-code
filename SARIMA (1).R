...
calc_accuracy <- function(actual, forecast_values) {
  ...
  c(
    ...
    MAPE = mean(
      abs((actual - forecast_values) / actual)
    ) * 100
  )
}

is_stable_fit <- function(model) {
  
  if (is.null(model$var.coef)) {
    return(FALSE)
  }
  ...
  for (q in 0:4) {
    for (P in 0:2) {
      for (Q in 0:2) {
        ...
        # Skip failed fit
        if (is.null(fit)) next
        # Skip numerically unstable fit
        if (!is_stable_fit(fit)) next
...
if (nrow(candidate_table) == 0) {
  ...
  stop(
    "No numerically stable SARIMA candidate could be fitted."
  )
}
...
cat(
  "Stable candidates fitted :",
  nrow(candidate_table),
  "\n"
)

cat(
  "Qualification            : Ljung-Box p-value > 0.05\n"
)

cat(
  "Ranking                  : Validation RMSE -> MAE -> MAPE -> AICc\n"
)

if (nrow(qualified) == 0) {
  
  cat(
    "No candidate passed Ljung-Box qualification.\n"
  )
  
  cat(
    "Fallback: rank all stable fitted candidates.\n"
  )
...
cat("\nTop 10 candidates before final refit stability check:\n")
...
# 7. SELECT FIRST MODEL THAT REMAINS STABLE
...
cat(
  "\n=== STABILITY CHECK AFTER TRAIN + VALIDATION REFIT ===\n"
)
...
for (i in seq_len(nrow(ranked))) {
  ...
  if (!is_stable_fit(refit)) {
    
    cat(
      "Rejected:",
      candidate$Model,
      "- unstable coefficient variance after refit\n"
    )
...
if (is.null(final_model)) {
  ...
  stop(
    "No numerically stable SARIMA model remained after Train + Validation refitting."
  )
}
...
cat(
  "Validation MAPE:",
  selected$Validation_MAPE,
  "%\n"
)

cat(
  "Ljung-Box p    :",
  selected$Ljung_Box_p,
  "\n"
)
...
cat(
  "\nCoefficient variance diagonal:\n"
)

print(
  diag(final_model$var.coef)
)

cat(
  "Stable variance check:",
  is_stable_fit(final_model),
  "\n"
)
...
if (
  is.finite(final_ljung_p) &&
  final_ljung_p > 0.05
) {
  
  cat(
    "Interpretation: no significant residual autocorrelation detected.\n"
  )
  
} else {
  
  cat(
    "Interpretation: residual autocorrelation may remain.\n"
  )
}
...
# 13. SAVE FINAL PARAMETERS
coef_variance <- diag(
  final_model$var.coef
)
...
if (!is_stable_fit(full_model)) {
  
  warning(
    "Full-data refit has unstable coefficient variance estimates."
  )
}
